...
function _toJson(excel, dest, head) {
  ...
  for (var i_sheet = 0; i_sheet < excel.worksheets.length; i_sheet++) {
    ...
    if (sheet.name.indexOf('@') == -1 && sheet.name[0] !== "#") {
      ...
      file_version[sheet.name + ".json"] = crypto.MD5(output).toString();
  ...
  file_version = JSON.stringify(file_version, null, 2);
  fs.writeFile(path.resolve(dest, "version_list.json"), file_version, function (err) {
  	if (err) {
  		console.log("error: ", err);
  		throw err;
  	}
  	console.log('built successfully -->  ', "version_list.json");
  })
}
